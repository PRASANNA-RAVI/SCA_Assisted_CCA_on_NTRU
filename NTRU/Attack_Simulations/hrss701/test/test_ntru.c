...
int main(void)
{
  ...
  for(uint32_t hg = 3; hg < limit_hg; hg=hg+3)
  {
    ...
    for(uint32_t hg1 = 3; hg1 < limit_hg; hg1=hg1+3)
    {
          ...
          uint32_t value1 = hg * (m_attack) + hg1 * 3 * (2*n_attack);
          ...
          if(value1 > q12 && (abs(q12 - value1) > C_VALUE_THRESHOLD_1))
          {
            ...
            {
              for(uint32_t poss1 = 0; poss1 <= 2*n_attack; poss1++)
              {
                    if(!(poss == m_attack && poss1 == (2*n_attack)))
                    {
                      uint32_t value2 = (hg * (poss) + 3 * hg1 * (poss1));
    ...
    for(int hg = 3; hg < limit_value; hg = hg+3)
    {
      ...
      for(int hg1 = 3; hg1 < limit_value; hg1 = hg1+3)
      {
        ...
        for(int hg2 = 3; hg2 < limit_value; hg2 = hg2+3)
        {
          ...
          int value1 = sample_1 * (m_attack) + 3 * sample_3 * (2*n_attack) + sample_2 * (1);
          int value2;
          ...
          if((value1 > q12) && (abs(value1 - q12) > GAP_THRESHOLD_1_1))
          {
            ...
            {
              for(int poss1 = 0; poss1 <= (2*n_attack); poss1++)
              {
                for(int poss2 = 0; poss2 <= 2; poss2++)
                {

                  if(!((poss == m_attack) && (poss1 == (2*n_attack)) && ((poss2 == 1) || (poss2 == 2))))
                  {
                    // value2 = sample_1 * (3*poss+poss1) + sample_2 * (3*poss2);

                    value2 = sample_1 * (poss) + 3 * sample_3 * (poss1) + sample_2 * (poss2);
    ...
    for(int hg = 3; hg < limit_value; hg = hg+3)
    {
      ...
      for(int hg1 = 3; hg1 < limit_value; hg1 = hg1+3)
      {
        ...
        for(int hg2 = 3; hg2 < limit_value; hg2 = hg2+3)
        {
          ...
          // int value1 = sample_1 * (3*2*m + 2*n) + sample_2 * (1*3);
          int value1 = sample_1 * (m_attack) + 3 * sample_3 * (2*n_attack) + sample_2 * (2);
          int value2;
            ...
            for(int poss = 0; poss <= m_attack; poss++)
            {
              for(int poss1 = 0; poss1 <= 2*n_attack; poss1++)
              {
                for(int poss2 = 0; poss2 <= 2; poss2++)
                {
                  ...
                  if(!((poss == m_attack) && (poss1 == (2*n_attack)) && (poss2 == 2)))
                  {
                    // value2 = sample_1 * (3*poss+poss1) + sample_2 * (3*poss2);
  ...
  for (int pq=0; pq<NO_TESTS; pq++)
  {
    ...
    for(sec_index = 0;  sec_index < NTRU_N; sec_index++)
    {
      ...
      if(sec_index == 100)
      {
        ...
        if((touch_0 < 20) || (touch_1 < 10) || (touch_2 < 2) || (abs(touch_0 - touch_1) < 20) || (abs(touch_2 - touch_1) < 3) || (touch_2 > touch_1) )
        {
          rejected = 1;
