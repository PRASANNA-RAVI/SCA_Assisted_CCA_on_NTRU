...
{
  int i;
  // printf("Inside poly_mod_3_Phi_n function\n");
  for(i=0; i <NTRU_N; i++)
  {
    // printf("%d, %d, %d, %d\n",r->coeffs[i],2*r->coeffs[NTRU_N-1],r->coeffs[i] + 2*r->coeffs[NTRU_N-1],mod3(r->coeffs[i] + 2*r->coeffs[NTRU_N-1]));
    r->coeffs[i] = mod3(r->coeffs[i] + 2*r->coeffs[NTRU_N-1]);
  }
}

void poly_mod_3_Phi_n_decrypt(poly *r)
{
  int i;
  // printf("Inside poly_mod_3_Phi_n function\n");
  for(i=0; i <NTRU_N; i++)
...
void poly_Rq_to_S3(poly *r, const poly *a)
{
  ...
  // printf("Printing cf mod 3 inside decrypt...\n");
  // for(int hfh = 0; hfh < NTRU_N; hfh++)
  // {
  //   // printf("[%d]: %d, ", hfh, mod3(r->coeffs[hfh]));
  //   printf("%d, ", mod3(r->coeffs[hfh]));
  //   // if(r->coeffs[hfh] > NTRU_Q/2)
  //   //   printf("[%d] = %d\n", hfh, r->coeffs[hfh]);
  // }
  // printf("\n");
