      ...
      for(int hg = 0; hg < q; hg++)
      {
        ...
        for(int hg1 = 0; hg1 < q; hg1++)
        {
              ...
              if(touch_np == 0)
              {
                ...
                if(max_distance + abs(value1 - q12) > max_distance2)
                {
                  c_value_1 = hg;
                  c_value_2 = hg1;
                  max_distance2 = max_distance + abs(value1 - q12);
                  printf("hg = %d, hg1 = %d, Diff1: %d, Diff2: %d\n", hg, hg1, abs(q12 - value1), max_distance);
                }
              }
