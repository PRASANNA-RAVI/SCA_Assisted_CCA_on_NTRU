...
static int get_random(void)
{
    ...
    unsigned int POLY_MASK_HERE_2 = 0x123BE412;
...
static int Weightw_mask(small *r)
{
  ...
  //
  // cur_value = (weight >> 8)&0xFF;
  // send_USART_bytes(&cur_value,1);
  //
  // cur_value = (weight >> 0)&0xFF;
  // send_USART_bytes(&cur_value,1);
...
static void Encrypt(Fq *c,const small *r,const Fq *h)
{
  ...
  if(global_function == 1)
  {
    ...
    int found_c_for_leakage = 0;
    int c_value_for_leakage;
    int actual_value;

    found_c_for_leakage = 0;
    while(found_c_for_leakage == 0)
    {
        c_value_for_leakage = Fq_freeze((get_random()*256 + get_random()));

        int actual_value = (3*c_value_for_leakage)%q;
        if(actual_value < 0)
          actual_value = actual_value + q;

        if((actual_value < q12) && (c_value_for_leakage%3 == 0))
        {
          found_c_for_leakage = 1;
        }
    }

    // while(found_num == 0)
    // {
    //   num = (get_random()*256+get_random())%q12;
    //
    //   if(num < 0)
    //     num = num + q12;
    //
    //   actual_value = (3*num);
    //
    //   if((actual_value > 0) && (actual_value < q12) && (num%3 == 0))
    //     found_num = 1;
    // }

    hr[0] = c_value_for_leakage;
  }
...
{
  // This is the operation that we try to get side-channel information from...
